...
void BinarySearch_float (uint32_t *iLeft, uint32_t *iRight, uint32_t iMax, float u, float *ValuesOnAxis)
{
	*iLeft = 0;
	*iRight = iMax;
	
	if (u<ValuesOnAxis[*iLeft]) {*iRight=*iLeft;}
	else if (u>ValuesOnAxis[*iRight]) {*iLeft=*iRight;}
	else {
		uint32_t i;
		while ((*iRight-*iLeft)>1) 
		{
			i = (*iRight+*iLeft)>>1;
			if (u<ValuesOnAxis[i]) {*iRight = i;}
			else {*iLeft = i;}
		}
	}
}
				...
				float *ValuesOnAxis_x, uint32_t	numberValueInArray_x)
{
	...
	BinarySearch_float (&iLeft, &iRight, tempValueInArray_x, input, ValuesOnAxis_x);
