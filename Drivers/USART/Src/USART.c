...
void USART_init(char *inBuffer)
{
	...
	gpio_af_set(USART_PORT, AltFuncNum,
		    USART_PIN_Tx); // configure the USART TX pin
	gpio_af_set(USART_PORT, AltFuncNum,
		    USART_PIN_Rx); // configure the USART RX pin
	/* configure USART TX as alternate function push-pull */
	gpio_mode_set(USART_PORT, GPIO_MODE_AF, GPIO_PUPD_PULLUP, USART_PIN_Tx);
	gpio_output_options_set(USART_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ,
				USART_PIN_Tx);
