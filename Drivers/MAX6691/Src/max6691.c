...
void timeToResist (uint32_t tHigh, uint32_t tLow, uint32_t rRef, uint16_t *rTherm)
{
	float div = (float)tHigh/(float)tLow;
	*rTherm = (uint16_t)(((float)rRef/(div-(float)0.0002))-(float)rRef);
}
							 ...
							 maxTempStruct *outStruct, uint8_t *timeReady, uint32_t *inputTimer)
{
	...
	{
		case MAX_IDLE:
			if ((*inputTimer>=100000)&(setStartBitTime==0)) { setStartBitTime=*inputTimer;}
			if (*inputTimer<(setStartBitTime+TIME_START_PULSE)) {gpio_bit_set(MAX_OUT_PORT ,MAX_OUT_PIN);}
			if (*inputTimer>=(setStartBitTime+TIME_START_PULSE)) {gpio_bit_reset(MAX_OUT_PORT ,MAX_OUT_PIN);}
		...
		case MAX_ERROR:
			if (*inputTimer>=TIME_FOR_ERR_RESET) {*SM = MAX_IDLE; *inputTimer = 0;}
		break;
...
MAX6691Status max6691_init()
{
	...
	gpio_output_options_set(MAX_IN_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, MAX_IN_PIN);
  ...
  //exti_interrupt_flag_clear(MAX_EXTI);
	...
	gpio_mode_set(MAX_OUT_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, MAX_OUT_PIN);
	gpio_output_options_set(MAX_OUT_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, MAX_OUT_PIN);
...
void MAX_IRQ_HANDLER (void) 
{
	...
	if (exti_interrupt_flag_get(MAX_EXTI)==SET) //if flag was set
	{
		...
		if (timeStart == 0) {timeStart = MAX_IRQ_inputTimer;}
		else 
		{
		...
		if (smMaxToInterrupt==MAX_T4_L) {timeStart = 0;} //reset the start time at the end of measurements
